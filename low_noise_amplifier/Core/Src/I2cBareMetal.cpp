...
uint8_t I2cBareMetal::byteReceive(char address, uint8_t bytes_to_read){

	i2c1_start(address, READ, bytes_to_read);
	...
	for (int i = 0; i < bytes_to_read; i++) {
		...
		while ((!READ_BIT(I2C1->ISR, I2C_ISR_RXNE)
				& !READ_BIT(I2C1->ISR, I2C_ISR_NACKF)) && j < 1000) {
			j++;
		}
		if (j == 1000) return 0;
...
void I2cBareMetal::byteTransmit(char address, char* buffer, uint8_t bytes_to_write){
	i2c1_start(address, WRITE, bytes_to_write);
	...
	for (int i = 0; i < bytes_to_write; i++) {
		...
		if (j == 1000) return;
		MODIFY_REG(I2C1->TXDR, I2C_TXDR_TXDATA, buffer[i]);
	}
...
void I2cBareMetal::i2c1_start(char saddr, uint8_t transfer_request, uint8_t N){
