...
#define LNA_VREF 3.28f
#define LNA_CURRENT_FACTOR 0.011f   // [mA]
#define TXBUFLEN 40
...
static const float LNA_MULTIPLIER = LNA_VREF / LNA_CURRENT_FACTOR;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    {
    	addr = i<<1;
        ret = HAL_I2C_IsDeviceReady(&hi2c1,addr,3,100);
        if(ret == HAL_OK)
        {
            i = 129;
        }
